...
│   ├── (account)/             # 로그인 사용자 라우트 그룹
│   │   └── mypage/            # 고객 정보와 주문 내역·상세 라우트
│   ├── products/              # 공개 상품 목록·상세 라우트
│   ├── coffee-guide/          # 1분 취향 테스트 라우트
│   ├── cart/                  # 장바구니 라우트
│   ├── checkout/              # 로그인 사용자 테스트 결제 라우트
│   ├── orders/                # 주문 완료 라우트
│   └── providers.tsx          # TanStack Query Provider
├── entities/
│   ├── customer/              # 고객 타입, 스키마, 주소·동의 필드
│   ├── order/                 # 고객 주문 타입과 주문 조회 API
...
│   ├── coffee-guide/          # 취향 응답, 후보 조회와 상품 추천
│   ├── filter-products/       # 상품 검색, 카테고리, 정렬과 URL 상태
│   ├── search-products/       # 헤더 상품 검색 다이얼로그와 URL 이동
│   └── cart/                  # 장바구니 상태, 수량 변경과 담기
├── widgets/
│   ├── home/                  # HomePage와 홈 섹션 조합
│   ├── coffee-guide/          # 취향 테스트 페이지 조합
│   ├── member/                # 회원 화면의 SSR 검사와 폼 조합
│   ├── product-list/          # 상품 목록 SSR 조회와 목록 UI 조합
│   ├── product-detail/        # 상품 상세 SSR 조회와 상세 UI 조합
│   ├── cart/                  # 장바구니 상품과 결제 예정 금액 조합
│   ├── checkout/              # 배송지·결제 수단·주문 금액 조합
│   ├── order-complete/        # 주문 완료 조회와 결과 화면
│   ├── order-history/         # 본인 주문 목록 SSR 조회와 페이지네이션
│   ├── order-detail/          # 본인 주문의 결제·배송·상태 상세
│   ├── account-nav/           # 마이페이지 공통 내비게이션
│   ├── site-header/           # 인증 상태와 모바일 메뉴를 반영하는 공통 헤더
│   └── site-footer/           # 공통 푸터
└── shared/
...
홈의 `1분 취향 테스트`에서 `/coffee-guide`로 이동해 추출 방식, 선호 풍미,
카페인 여부를 선택할 수 있습니다. 마지막 응답 후 기존 공개 상품 API에서
후보를 조회하고, 판매 가능 수량과 디카페인 여부를 확인한 뒤 풍미 설명을
점수화해 최대 3개 상품을 추천합니다. 같은 원두의 용량별 상품은 하나로
묶습니다. 선택한 추출 방식에 디카페인 상품이 없으면 다른 방식의 상품을
대신 보여 주되, 결과 화면에 대체 추천임을 표시합니다. 추천 카드에서는
기존 상품 상세 페이지로 이동합니다.

모바일 헤더는 검색과 장바구니를 바로 노출하고, 메뉴 버튼에서 주요 메뉴와
로그인 상태에 맞는 주문 내역·내 정보·로그아웃 링크를 제공합니다.
...
장바구니는 `features/cart`에서 Zustand 상태와 사용자 행동을 관리하고
`widgets/cart`에서 장바구니 페이지를 조합합니다.

- 상품 상세에서 수량을 선택해 장바구니에 담습니다.
- 동일 상품은 판매 가능 수량을 넘지 않는 범위에서 수량을 합산합니다.
- 헤더에서 전체 상품 수량을 확인할 수 있습니다.
- 장바구니에서 수량 변경, 상품 삭제와 전체 비우기를 지원합니다.
- 상품 금액, 배송비, 무료배송까지 남은 금액과 총 금액을 계산합니다.
- 비로그인 장바구니는 Zustand persist를 사용해 브라우저에 유지합니다.
- 로그인하면 비로그인 장바구니 수량을 기존 Supabase 장바구니에 합치고,
  서버가 재고에 맞게 보정한 결과로 브라우저 상태를 교체합니다.
- 로그인 상태의 수량 변경, 상품 삭제와 전체 비우기는 짧게 모아서
  Supabase 장바구니 스냅샷으로 저장합니다.
- 저장 도중 다시 수량이 바뀌어도 요청을 순서대로 처리하며, 실패한 변경은
  브라우저에 남겨 다음 접속 또는 온라인 복귀 때 다시 동기화합니다.
- 로그아웃하면 로그인 계정의 장바구니가 브라우저에 남지 않도록 로컬 상태를
  비웁니다. 서버 장바구니는 유지되므로 다음 로그인 때 다시 불러옵니다.
- 주문 생성 전에는 현재 상품의 재고를 서버에서 다시 검증해야 합니다.

### 테스트 결제

장바구니의 `주문하기` 버튼은 로그인 사용자 전용 `/checkout`으로 이동합니다.
비로그인 사용자는 로그인 후 원래 주문서로 돌아옵니다.

- 고객의 기본 배송지를 불러오되 현재 주문에서 수정할 수 있습니다.
- 실제 결제 정보 없이 테스트 카드와 테스트 카카오페이를 선택합니다.
- React Hook Form과 Zod로 배송지 및 테스트 결제 확인 여부를 검증합니다.
- Supabase RPC가 상품 가격과 재고를 다시 확인한 뒤 주문, 주문 상품, 결제,
  배송과 상태 이력을 하나의 트랜잭션으로 생성합니다.
- 결제된 상품 수량은 예약 재고에 반영하고 고객의 서버 장바구니를 비웁니다.
- 같은 결제 요청 ID를 재사용하면 기존 주문을 반환하여 재시도 시 중복 주문을
  방지합니다.
- 주문 완료 페이지는 현재 로그인한 사용자의 테스트 주문만 조회합니다.
- 테스트 결제는 포트폴리오 기능이며 실제 PG 승인이나 금액 청구가 발생하지
  않습니다.

### 주문 내역과 상세

로그인 사용자는 `/mypage/orders`에서 본인의 주문만 최신순으로 확인하고,
`/mypage/orders/[orderNumber]`에서 주문 상품, 결제, 배송과 상태 변경 이력을
확인합니다.

- 주문 목록의 페이지는 URL Search Params로 관리하며 서버 RPC가 페이지 단위로
  조회합니다.
- 주문 상세는 Server Component에서 조회하고 존재하지 않거나 다른 사용자의
  주문 번호이면 404로 처리합니다.
- 주문 목록과 상세 RPC는 모두 `auth.uid()`로 소유권을 확인합니다. 클라이언트가
  전달한 사용자 ID는 신뢰하지 않습니다.
- `/mypage` 공통 레이아웃에서 주문 내역과 내 정보 내비게이션을 제공합니다.
- 목록, 상세, 로딩, 오류와 빈 상태를 각각 라우트 또는 위젯 수준에서 처리합니다.
