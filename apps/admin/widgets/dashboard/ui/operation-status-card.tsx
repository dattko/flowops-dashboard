...
export const OperationStatusCard = ({ todayOrderSummary }: OperationStatusCardProps) => {
  return (
    <Card className="gap-0 rounded-none border border-[#e3e0d8] bg-white py-0 shadow-[0_1px_2px_rgba(42,39,31,0.03)] ring-0">
      ...
      <div className="flex items-center justify-between">
        <SectionHeading
          title="오늘 처리 현황"
          description={`전체 주문 ${todayOrderSummary.totalOrders}건 기준`}
        />
      ...
      <div className="mt-6 flex items-center gap-6">
        <div
          ...
          style={{
            background: `conic-gradient(#d7753f 0 ${todayOrderSummary.completionRate}%, #eeeae1 ${todayOrderSummary.completionRate}% 100%)`,
          }}
          role="img"
          aria-label={`오늘 주문 처리율 ${todayOrderSummary.completionRate}퍼센트`}
        >
          <div className="grid size-[86px] place-items-center rounded-full bg-white text-center">
            <div>
              <Typography variant="metric">{todayOrderSummary.completionRate}%</Typography>
              <Typography variant="caption" tone="muted">처리 완료</Typography>
            </div>
          </div>
        </div>
        <dl className="min-w-0 flex-1 space-y-3">
          <StatusRow label="배송 완료" value={todayOrderSummary.completedCount.toString()} color="bg-[#3d8672]" />
          <StatusRow label="배송 중" value={todayOrderSummary.shippingCount.toString()} color="bg-[#d6b76d]" />
          <StatusRow label="처리 대기" value={todayOrderSummary.pendingCount.toString()} color="bg-[#ddd9cf]" />
        </dl>
      </div>
      </CardContent>
    </Card>
