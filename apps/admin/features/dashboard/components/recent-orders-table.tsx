        ...
        <table className="w-full min-w-[780px] text-left">
          ...
          <tbody className="divide-y divide-[#f0eee8]">
            {recentOrders.map((order) => (
              <tr key={order.id} className="type-label transition-colors hover:bg-[#faf9f6]">
                <td className="px-6 py-3.5 font-medium text-muted-foreground">{order.id}</td>
                <td className="px-4 py-3.5 font-semibold text-foreground">{order.customer}</td>
                <td className="max-w-[260px] truncate px-4 py-3.5 text-muted-foreground">{order.product}</td>
                <td className="px-4 py-3.5 text-muted-foreground">{order.channel}</td>
                <td className="px-4 py-3.5 font-semibold text-foreground">{order.amount}</td>
                <td className="px-4 py-3.5">
                  <StatusBadge className={ORDER_STATUS_STYLES[order.status]}>
                    {order.status}
                  </StatusBadge>
                </td>
                <td className="px-6 py-3.5 text-right text-muted-foreground">{order.time}</td>
              </tr>
            ))}
          </tbody>
        </table>
